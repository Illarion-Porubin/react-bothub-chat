...
const Header: React.FC = () => {
  ...
  const lalanguageList = ["ru", "en", "es", "fr", "pt"];

  const handlerLang = (value: string) => {
    setLang(value)
    setPopupLang(false)
  }
      ...
      <div className={style.header}>
        <div className="container fix">
          ...
          <div className={style.buttons}>
            <button className={style.headerLang} onClick={() => setPopupLang(prev => !prev)}>
              <svg
                viewBox="0 0 18 18"
                width="18"
                height="18"
            ...
            <motion.div
                className={style.langList}
                animate={{
                  opacity: popupLang ? 1 : 0,
                  visibility: popupLang ? "visible" : "hidden",
                }}
              >
                {lalanguageList.map((item, id) => (
                  <button className={style.langBtn} key={id} onClick={() => handlerLang(item)}>
                    {item}
                  </button>
                ))}
              </motion.div>
